...
async function getClickUpToken(userId: string): Promise<string | null> {
  const [row] = await db
    .select()
    .from(settings)
    .where(and(eq(settings.userId, userId), eq(settings.key, "clickup-api-token")))
  const value = row?.value
  return typeof value === "string" && value.length > 0 ? value : null
}

async function proxyToClickUp(request: NextRequest, { params }: RouteParams): Promise<Response> {
  ...
  if (!token) {
    return NextResponse.json({ error: "ClickUp API token not configured" }, { status: 422 })
  }
  ...
  try {
    ...
    console.error(`[clickup-proxy] ${request.method} /${clickUpPath} failed: ${message}`)
    return NextResponse.json({ error: "ClickUp proxy error", detail: message }, { status: 502 })
  }
}
