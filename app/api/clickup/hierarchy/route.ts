...
import type { ClickUpSpace, ClickUpFolder, ClickUpList, ClickUpView } from "@/types"
...
export async function GET(): Promise<Response> {
  ...
  try {
    ...
    const [spacesData, viewsData] = await Promise.all([
      fetchJson<{ spaces: RawSpace[] }>(`${CLICKUP_API_BASE}/team/${teamId}/space?archived=false`, token),
      fetchJson<{ views: RawView[] }>(`${CLICKUP_API_BASE}/team/${teamId}/view`, token),
    ])
    ...
    const spaces: ClickUpSpace[] = await Promise.all(
      spacesData.spaces.map(async (space) => {
        const [foldersData, listsData] = await Promise.all([
          fetchJson<{ folders: RawFolder[] }>(`${CLICKUP_API_BASE}/space/${space.id}/folder?archived=false`, token),
          fetchJson<{ lists: ClickUpList[] }>(`${CLICKUP_API_BASE}/space/${space.id}/list?archived=false`, token),
        ])
    ...
    return NextResponse.json({ error: "Failed to fetch hierarchy", detail: message }, { status: 502 })
  }
}
