...
export async function POST(request: NextRequest) {
  ...
  const { workspaceId, filePath, startLine, endLine, body: commentBody, contentSnapshot } = body as {
    workspaceId: string
    filePath: string
    startLine: number
    endLine: number
  ...
  if (!workspaceId || !filePath || startLine == null || endLine == null || !commentBody) {
    return NextResponse.json(
      { error: "workspaceId, filePath, startLine, endLine, and body are required" },
      { status: 400 }
    )
  }
