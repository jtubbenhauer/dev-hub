...
function isCreateResult(
  value: unknown,
): value is WorkspaceProviderCreateResult {
  if (!value || typeof value !== "object") return false;
  const obj = value as Record<string, unknown>;
  ...
  return (
    typeof endpoints.opencode === "string" &&
    typeof endpoints.agent === "string"
  );
}
...
async function verifyAgentHealth(agentUrl: string): Promise<boolean> {
  try {
    const response = await globalThis.fetch(
      new URL("/health", agentUrl).toString(),
      {
...
export async function POST(request: NextRequest) {
  ...
  const linkedTaskId =
    typeof body.linkedTaskId === "string" ? body.linkedTaskId : null;
  const linkedTaskMeta =
    body.linkedTaskMeta && typeof body.linkedTaskMeta === "object"
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      child.on("close", (exitCode, signal) => {
        if (exitCode === null) {
          emit("error", {
            message: `Provider command was terminated by signal ${signal}`,
          });
          controller.close();
          return;
        }
        if (exitCode !== 0) {
          emit("error", {
            message: `Provider command exited with code ${exitCode}`,
          });
          controller.close();
          return;
        }
...
async function handlePostSpawn({
  provider,
  id,
  repo,
  branch,
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (attempt < maxAttempts) {
      emit("status", {
        message: `Agent not ready yet, retrying... (${attempt}/${maxAttempts})`,
      });
