...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  const result = await db
    ...
    .where(
      and(eq(reviews.id, reviewId), eq(workspaces.userId, session.user.id)),
    );
  ...
  const [file] = await db
    ...
    .where(
      and(
        eq(reviewFiles.id, Number(fileId)),
        eq(reviewFiles.reviewId, reviewId),
      ),
    );
