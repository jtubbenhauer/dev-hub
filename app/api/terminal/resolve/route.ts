...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json(
    { error: "No shell command configured. Set a shell command on the provider or workspace." },
    { status: 422 }
  )
}
...
async function findProvider(
  ...
  const [settingRow] = await db
    .select()
    .from(settings)
    .where(and(eq(settings.userId, userId), eq(settings.key, "workspace-providers")))
