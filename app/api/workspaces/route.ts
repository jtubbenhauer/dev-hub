...
function detectWorkspaceType(dirPath: string): "repo" | "worktree" {
...
async function createLocalWorkspace(
  ...
  const parentRepoPath =
    type === "worktree" ? findParentRepoPath(resolvedPath) : null;
...
async function createRemoteWorkspace(
  ...
  if (!isHealthy) {
    return NextResponse.json(
      {
        error:
          "Cannot reach agent at the provided URL. Ensure the agent is running.",
      },
      { status: 400 },
    );
  }
  ...
  const providerMeta =
    typeof body.providerMeta === "object" && body.providerMeta !== null
      ? body.providerMeta
      : null;

  const shellCommand =
    typeof body.shellCommand === "string" ? body.shellCommand : null;

  // For remote workspaces, path is informational (the container's workspace path)
  const workspacePath =
    typeof body.path === "string" ? body.path : "/workspace";
