...
const McpServerRow = memo(function McpServerRow({ name, value }: { name: string; value: McpStatusValue }) {
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <div className="flex items-center gap-2 rounded px-1.5 py-1 text-xs">
          <span className={`size-1.5 rounded-full shrink-0 ${statusDotColor(value.status)}`} />
...
export const McpStatusPanel = memo(function McpStatusPanel() {
  ...
  const connectedCount = entries.filter(([, v]) => v.status === "connected").length
  const errorCount = entries.filter(
    ([, v]) => v.status === "failed" || v.status === "needs_auth" || v.status === "needs_client_registration"
  ).length

  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        <span>{connectedCount}/{entries.length} active</span>
        {errorCount > 0 && (
