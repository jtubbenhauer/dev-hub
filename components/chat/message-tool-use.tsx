...
export const MessageToolUse = memo(
  function MessageToolUse({ part, nested }: MessageToolUseProps) {
...
function StandardToolCall({
  ...
  return (
    <div
      className={cn(
        "w-full min-w-0 overflow-hidden py-0.5",
        nested ? "pl-1" : "border-muted-foreground/30 border-l-2 py-1 pl-3",
      )}
    >
      {nested && (
        <span className="text-muted-foreground/40 mr-1 text-xs select-none">
          └
        </span>
      )}
      <button
      ...
      >
        {isExpanded ? (
          ...
          <ChevronRight className="text-muted-foreground/60 size-3 shrink-0" />
        )}
  ...
  return (
    <div
      className={cn(
        ...
        nested ? "py-0.5 pl-2" : "border-l-2 border-violet-500/50 py-1.5 pl-3",
      )}
    >
      <button
        type="button"
...
function ToolOutput({
  ...
  return (
    <div className="bg-muted/50 overflow-hidden rounded">
      <pre
        className={cn(
          "overflow-x-auto p-2 font-mono text-xs break-words whitespace-pre-wrap",
          lang && !hasAnsi && "text-muted-foreground",
        )}
      >
        {lang && (
          <code className="text-muted-foreground/50 mb-1 block text-[10px] tracking-wider uppercase">
            {lang}
          </code>
        )}
        {hasAnsi ? (
