      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-1">
            ...
            <span
              className={`text-xs font-medium ${thresholdColor(current.cpu.usage)}`}
            >
              {current.cpu.usage}%
            </span>
          </CardHeader>
          <CardContent className="pt-0">
            ...
            <p className="text-muted-foreground mb-2 truncate text-xs">
              ...
              {current.cpu.temperature !== undefined &&
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-1">
            ...
            <span
              className={`text-xs font-medium ${thresholdColor(current.memory.usagePercent)}`}
            >
          ...
          <CardContent className="pt-0">
            <div className="mb-1">
              <span className="text-2xl font-bold">
                {formatBytes(current.memory.used)}
              </span>
            ...
            {current.memory.swapTotal > 0 && (
              <p className="text-muted-foreground mb-2 text-xs">
                Swap: {formatBytes(current.memory.swapUsed)} /{" "}
                {formatBytes(current.memory.swapTotal)}
              </p>
            )}
            {current.memory.swapTotal === 0 && (
            ...
            {primaryDisk ? (
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground text-xs">
                  ...
                  {formatBytes(primaryDisk.size)} · {primaryDisk.mount}
                </p>
                {current.disks.slice(1).map((disk) => (
                  <div key={disk.mount} className="space-y-0.5">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className={thresholdColor(disk.usagePercent)}>
                        {disk.usagePercent}%
                      </span>
                    </div>
              ...
              <p className="text-muted-foreground text-sm">No disks detected</p>
            )}
          </CardContent>
        </Card>
            ...
            <div className="mb-1 space-y-0.5">
              <div className="flex items-center gap-1.5 text-sm">
                ...
                <span className="font-semibold">
                  {formatBytes(totalRxSec)}/s
                </span>
              </div>
              <div className="flex items-center gap-1.5 text-sm">
                <span className="text-muted-foreground text-xs">↑</span>
                <span className="font-semibold">
                  {formatBytes(totalTxSec)}/s
                </span>
              </div>
            </div>
            <p className="text-muted-foreground mb-2 text-xs">
