    ...
    <div className="p-1 space-y-1">
      {Array.from({ length: 8 }, (_, i) => (
        <div key={i} className="flex items-center gap-2 px-2 py-1.5">
          ...
          <Skeleton className="h-3 rounded ml-auto" style={{ width: `${20 + (i % 2) * 10}%` }} />
        </div>
      ))}
    </div>
  )
...
export function FilePickerDialog() {
  ...
  const [pickerWorkspaceId, setPickerWorkspaceId] = useState<string | null>(activeWorkspaceId)
  ...
  const handleKeyDown = useCallback(
      ...
      switch (e.key) {
        ...
        case "Tab":
          ...
          if (workspaces.length > 1) {
            e.preventDefault()
            const currentIdx = workspaces.findIndex((w) => w.id === pickerWorkspaceId)
    ...
    [results, selectedIndex, handleSelect, close, workspaces, pickerWorkspaceId]
  )
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && close()}>
      ...
      >
        <DialogHeader className="sr-only">
          ...
          <DialogDescription>Fuzzy search across all workspace files</DialogDescription>
        </DialogHeader>
