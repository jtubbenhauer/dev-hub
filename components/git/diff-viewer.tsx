...
export function DiffViewer({
  ...
  const [commentInputLine, setCommentInputLine] =
    useState<CommentInputLine | null>(null);
  ...
  const uniqueFilePaths = useMemo(
    () =>
      Array.from(
        new Set(lines.map((l) => l.filePath).filter(Boolean) as string[]),
      ),
    [lines],
  );
  ...
  return (
    <ScrollArea className="min-h-0 flex-1">
      {fileName && (
        <div className="bg-muted/50 text-muted-foreground sticky top-0 z-10 border-b px-3 py-1.5 font-mono text-xs">
          {fileName}
        </div>
      )}
      <div className="font-mono text-xs leading-5">
        {lines.map((line, index) => {
          ...
          const isHovered =
            canComment &&
            hoveredLine?.lineNumber === lineNumber &&
            hoveredLine?.filePath === line.filePath;
          const lineComments =
          ...
          return (
            <div key={index}>
              ...
              >
                <span className="border-border/50 text-muted-foreground/50 w-12 shrink-0 border-r px-1.5 text-right select-none">
                  {line.oldLineNumber ?? ""}
                </span>
                <span className="border-border/50 text-muted-foreground/50 w-12 shrink-0 border-r px-1.5 text-right select-none">
                  {line.newLineNumber ?? ""}
                ...
                <span className="flex-1 px-1 break-all whitespace-pre-wrap">
                  {line.content}
                </span>
                {canComment && lineNumber !== null && line.filePath && (
                  <span className="flex shrink-0 items-center gap-0.5 pr-1">
                    {hasComments && (
                      <Button
                        ...
                        title="View comments"
                        onClick={() =>
                          handleToggleCommentThread(lineNumber, line.filePath!)
                        }
              ...
              {isCommentThreadOpen && lineComments.length > 0 && (
                <div className="mx-2 mb-1">
                  <CommentThread
                    ...
                    onResolve={(id) =>
                      resolveComment.mutate({ id, resolved: true })
                    }
