  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button className="hover:bg-accent/50 flex items-center gap-2.5 rounded-md px-2 py-1 text-xs transition-colors">
          <span className="flex items-center gap-1">
            <svg
              className={cn("size-2", dotColor(cpuPercent))}
              viewBox="0 0 8 8"
            >
          ...
          <span className="flex items-center gap-1">
            <svg
              className={cn("size-2", dotColor(memPercent))}
              viewBox="0 0 8 8"
            >
              <circle cx="4" cy="4" r="4" />
            </svg>
      ...
      <PopoverContent align="end" className="w-56 space-y-2.5 p-3">
        <div className="space-y-1.5">
          ...
          <div className="text-muted-foreground flex items-center justify-between text-xs">
            ...
            <span className={cn("font-medium", usageColor(cpuPercent))}>
              {cpuPercent}%
            </span>
          </div>
          <div className="text-muted-foreground flex items-center justify-between text-xs">
            <span>
              {stats.cpu.cores} cores · {stats.cpu.speed.toFixed(1)} GHz
            </span>
        ...
        <div className="space-y-1.5">
          ...
          <div className="text-muted-foreground flex items-center justify-between text-xs">
            ...
            <span className={cn("font-medium", usageColor(memPercent))}>
              {memPercent}%
            </span>
          </div>
          {stats.memory.swapTotal > 0 && (
            <div className="text-muted-foreground flex items-center justify-between text-xs">
              <span>
                Swap {formatBytes(stats.memory.swapUsed)} /{" "}
