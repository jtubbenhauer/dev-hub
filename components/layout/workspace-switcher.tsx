...
function useWorkspaceBranches(
  ...
  const results = useQueries({
    queries: workspaces.map((w) => {
      const isInactiveRemote =
        w.backend === "remote" && w.id !== activeWorkspaceId;
      const shouldDisable =
...
export function WorkspaceSwitcher() {
  ...
  const healthStatuses = useRemoteWorkspaceHealthStatuses(
    workspaces,
    activeWorkspaceId,
  );
  const branches = useWorkspaceBranches(workspaces, activeWorkspaceId);

  const [wakingWorkspaceId, setWakingWorkspaceId] = useState<string | null>(
    null,
  );
              ...
              <div className="flex items-center gap-2">
                ...
                {supportsAutoSuspend && (
                  <>
                    ...
                    {isWaking && (
                      <span className="shrink-0 animate-pulse rounded-full bg-yellow-500/20 px-1.5 py-0.5 text-[10px] text-yellow-600 dark:text-yellow-400">
                        Waking...
                      </span>
                    )}
                    {health === "healthy" && !isWaking && (
                      <span className="shrink-0 rounded-full bg-green-500/20 px-1.5 py-0.5 text-[10px] text-green-600 dark:text-green-400">
                        Active
                      </span>
                    )}
                    {health === "unreachable" && !isWaking && (
                      <span className="shrink-0 rounded-full bg-red-500/20 px-1.5 py-0.5 text-[10px] text-red-600 dark:text-red-400">
                        Error
                      </span>
                    )}
                  </>
