...
const SessionPickerContext = createContext<SessionPickerContextValue | null>(
  null,
);
...
export function useSessionPicker() {
  ...
  if (!ctx)
    throw new Error(
      "useSessionPicker must be used within SessionPickerProvider",
    );
  return ctx;
}

export function SessionPickerProvider({
  children,
}: {
...
export function SessionPickerDialog() {
  ...
  const handleSelect = useCallback(
    (index: number) => {
      ...
      if (result) {
        const entry = query
          ? (result as { entry: SessionEntry; match: FuzzyMatch }).entry
          : (result as unknown as SessionEntry);
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && close()}>
      ...
      >
        <DialogHeader className="sr-only">
          ...
          <DialogDescription>
            Search and switch between chat sessions
          </DialogDescription>
        </DialogHeader>
        ...
        <ScrollArea className="h-[min(400px,60vh)] [&>[data-slot=scroll-area-viewport]]:!overflow-x-hidden [&>[data-slot=scroll-area-viewport]>div]:!block">
          <div ref={listRef} className="p-1">
                    ...
                    return (
                      <span className="text-muted-foreground flex shrink-0 items-center gap-1 text-[11px]">
                        <span
                          ...
                          style={{
                            backgroundColor:
                              wsInfo.color ?? "var(--color-muted-foreground)",
                          }}
                        />
            ...
            {results.map((result, i) => {
              ...
              const status =
                workspaceStates[entry.workspaceId]?.sessionStatuses[
                  entry.session.id
                ];
                ...
                >
                  {isBusy ? (
                    <Brain
                      ...
                      style={
                        entry.workspaceColor
                          ? { color: entry.workspaceColor }
                    ...
                    <MessageSquare
                      className={cn(
                        "size-3.5 shrink-0",
                        !entry.workspaceColor && "text-muted-foreground",
                      )}
                      style={
                        entry.workspaceColor
                          ? { color: entry.workspaceColor }
                  ...
                  <div
                    className={cn(
                      ...
                      isCurrentSession && "font-semibold",
                    )}
                  >
