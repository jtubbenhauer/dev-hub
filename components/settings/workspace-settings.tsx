...
export function WorkspaceSettings() {
  ...
  const { defaultWorkspaceId, isLoading: isLoadingDefault } =
    useDefaultWorkspaceSetting();
  const { worktreeBaseDir, isLoading: isLoadingWorktree } =
    useWorktreeBaseDirSetting();
  const { cloneBaseDir, isLoading: isLoadingClone } = useCloneBaseDirSetting();
  const { isAutoColorEnabled, isLoading: isLoadingAutoColor } =
    useAutoColorSetting();
  ...
  const handleDefaultWorkspaceChange = (value: string) => {
    ...
    mutation.mutate(
      { key: SETTINGS_KEYS.DEFAULT_WORKSPACE, value: next },
      {
        onSuccess: () =>
          toast.success(
            next ? "Default workspace set" : "Default workspace cleared",
          ),
      },
    );
  };
  ...
  const handleAutoColorToggle = (checked: boolean) => {
    mutation.mutate(
      ...
      {
        onSuccess: () =>
          toast.success(checked ? "Auto-color enabled" : "Auto-color disabled"),
      },
    );
  };
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              disabled={
                localWorktreeDir === worktreeBaseDir || mutation.isPending
              }
            >
              Save
            </Button>
