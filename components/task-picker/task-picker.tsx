...
export function useTaskPicker() {
  ...
  if (!ctx)
    throw new Error("useTaskPicker must be used within TaskPickerProvider");
  return ctx;
}

export function TaskPickerProvider({
  children,
}: {
  ...
  const handleSelectTask = useCallback(
    (task: ClickUpTask) => {
      ...
      window.dispatchEvent(
        new CustomEvent("devhub:select-task", {
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && close()}>
      ...
      >
        <DialogHeader className="sr-only">
          ...
          <DialogDescription>
            Search and switch between ClickUp tasks
          </DialogDescription>
        </DialogHeader>
