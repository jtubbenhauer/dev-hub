...
async function parseShellAliases(
  customRcPath?: string,
...
async function parsePackageScripts(
...
async function parseMakeTargets(
  workspacePath: string,
): Promise<AutocompleteSuggestion[]> {
...
async function parseCargoTargets(
  workspacePath: string,
): Promise<AutocompleteSuggestion[]> {
...
export async function getSuggestions(
  ...
  if (userId) {
    const [row] = await db
      ...
      .from(settings)
      .where(
        and(eq(settings.userId, userId), eq(settings.key, "shell-rc-path")),
      )
      .limit(1);
    if (row?.value) {
  ...
  for (const s of all) {
    if (!seen.has(s.value)) {
      seen.set(s.value, {
        ...s,
        frequency: freqMap.get(s.value) ?? s.frequency,
      });
    }
  }
