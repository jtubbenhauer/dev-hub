...
export function parseCommentRefs(text: string): {
  refs: CommentRef[];
  cleanedText: string;
} {
...
export function detachCommentFromChat(commentId: number): void {
  window.dispatchEvent(
    new CustomEvent("detach-comment-from-chat", { detail: { commentId } }),
  );
}

export function updateCommentInChat(commentId: number, body: string): void {
  window.dispatchEvent(
    new CustomEvent("update-comment-in-chat", { detail: { commentId, body } }),
  );
}

export function getAllCachedComments(
  queryClient: {
    getQueriesData: <T>(opts: {
...
export function removePendingCommentChip(
  workspaceId: string,
  id: number,
): void {
  const updated = getPendingCommentChips(workspaceId).filter(
    (c) => c.id !== id,
  );
  if (updated.length === 0) {
