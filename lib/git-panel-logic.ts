...
export function sortFiles(files: ReviewChangedFile[], mode: SortMode): ReviewChangedFile[] {
  const sorted = [...files]
  switch (mode) {
    case "name-asc":
...
export function buildFlatFiles(
  ...
  return [
    ...
    ...sort(status.untracked.map((p) => ({ path: p }))).map((f) => ({ path: f.path, isStaged: false })),
    ...sort(status.conflicted.map((p) => ({ path: p }))).map((f) => ({ path: f.path, isStaged: false })),
  ]
}
