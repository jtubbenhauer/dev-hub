import type { ActivationKeyConfig, ActivationKeyPreset } from "@/types/leader-key"
...
export const ACTIVATION_KEY_PRESETS: ActivationKeyPreset[] = [
  ...
  { id: "ctrl+shift+space", config: { key: " ", ctrlKey: true, shiftKey: true } },
]
...
export function matchesActivationKey(e: KeyboardEvent, config: ActivationKeyConfig): boolean {
...
export function formatActivationKey(config: ActivationKeyConfig, isMac?: boolean): string {
...
export function isActivationKeyEqual(a: ActivationKeyConfig, b: ActivationKeyConfig): boolean {
  return (
    a.key === b.key &&
    !!a.ctrlKey === !!b.ctrlKey &&
    !!a.altKey === !!b.altKey &&
...
export function findMatchingPresetId(config: ActivationKeyConfig): string | null {
  const match = ACTIVATION_KEY_PRESETS.find((p) => isActivationKeyEqual(p.config, config))
  return match?.id ?? null
}

export function isValidActivationKeyConfig(value: unknown): value is ActivationKeyConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false
  const obj = value as Record<string, unknown>
  if (typeof obj.key !== "string" || obj.key.length === 0) return false
  const hasModifier = !!obj.ctrlKey || !!obj.altKey || !!obj.shiftKey || !!obj.metaKey
  return hasModifier
}
