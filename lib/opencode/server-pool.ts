...
async function tryAdoptExistingServer(port: number): Promise<ServerState | null> {
...
function waitForServerUrl(proc: ChildProcess, timeoutMs: number): Promise<string> {
  return new Promise((resolve, reject) => {
    const timeoutId = setTimeout(() => {
      reject(new Error(`OpenCode server startup timed out after ${timeoutMs}ms`))
    }, timeoutMs)
...
export async function getOrStartServer(): Promise<{ url: string; port: number }> {
