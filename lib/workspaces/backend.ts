...
export interface WorkspaceBackend {
  ...
  push(remote?: string, branch?: string, setUpstream?: boolean): Promise<string>
  ...
  addWorktree(branch: string, newBranch: boolean, basePath?: string, startPoint?: string): Promise<string>
...
export class LocalBackend implements WorkspaceBackend {
  ...
  push(remote?: string, branch?: string, setUpstream?: boolean) {
    return push(this.workspacePath, remote ?? "origin", branch, setUpstream ?? false)
  }
  ...
  addWorktree(branch: string, newBranch: boolean, basePath?: string, startPoint?: string) {
    return addWorktree(this.workspacePath, branch, newBranch, basePath, startPoint)
  }
...
export class RemoteBackend implements WorkspaceBackend {
  ...
  private async agentGet<T>(path: string, params?: Record<string, string>): Promise<T> {
  ...
  private async agentPost<T>(path: string, body?: Record<string, unknown>): Promise<T> {
    const url = new URL(path, this.agentUrl)
    const response = await globalThis.fetch(url.toString(), {
      method: "POST",
  ...
  async readFile(filePath: string) {
    return this.agentGet<{ content: string; size: number }>("/files/read", { path: filePath })
  }
  ...
  async getFileStatuses() {
    const result = await this.agentGet<Record<string, FileGitStatus>>("/files/status")
  ...
  async commit(message: string) {
    const result = await this.agentPost<{ hash: string }>("/git/commit", { message })
    return result.hash
  }

  async push(remote?: string, branch?: string, setUpstream?: boolean) {
    const result = await this.agentPost<{ result: string }>("/git/push", { remote, branch, setUpstream })
    return result.result
  }

  async pull(remote?: string, branch?: string) {
    const result = await this.agentPost<{ result: string }>("/git/pull", { remote, branch })
    return result.result
  }
  ...
  async getOriginalContent(filePath: string) {
    const result = await this.agentGet<{ content: string }>("/git/content", { path: filePath, ref: "HEAD" })
    return result.content
  }

  async getCurrentContent(filePath: string) {
    const result = await this.agentGet<{ content: string }>("/files/read", { path: filePath })
    return result.content
  }

  async getContentAtRef(ref: string, filePath: string) {
    const result = await this.agentGet<{ content: string }>("/git/content", { path: filePath, ref })
    return result.content
  }

  async getStagedContent(filePath: string) {
    const result = await this.agentGet<{ content: string }>("/git/content", { path: filePath, ref: "staged" })
    return result.content
  }

  async getChangedFiles(baseRef: string) {
    return this.agentGet<ReviewChangedFile[]>("/git/changed-files", { baseRef })
  }
  ...
  async getMergeBase(ref: string) {
    const result = await this.agentGet<{ mergeBase: string }>("/git/merge-base", { ref })
    return result.mergeBase
  }

  async computeDiffHash(baseRef: string, filePath: string) {
    const result = await this.agentGet<{ hash: string }>("/git/diff-hash", { baseRef, path: filePath })
    return result.hash
  }

  async computeUncommittedDiffHash(filePath: string) {
    const result = await this.agentGet<{ hash: string }>("/git/uncommitted-diff-hash", { path: filePath })
    return result.hash
  }
  ...
  async addWorktree(branch: string, newBranch: boolean, basePath?: string, startPoint?: string) {
    const result = await this.agentPost<{ path: string }>("/git/worktrees/add", {
      branch,
      newBranch,
      basePath,
      startPoint,
  ...
  async removeWorktree(worktreePath: string, force?: boolean) {
    await this.agentPost("/git/worktrees/remove", { path: worktreePath, force })
  }
...
export function getBackend(workspace: Workspace): WorkspaceBackend {
  if (workspace.backend === "remote") {
    if (!workspace.agentUrl || !workspace.opencodeUrl) {
      throw new Error(`Remote workspace "${workspace.name}" is missing agent or opencode URLs`)
    }
