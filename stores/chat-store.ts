...
export const useChatStore = create<ChatState>()((set, get) => ({
  ...
  restoreSessionLocal: (snapshot) => {
    set((state) => {
      const ws =
        state.workspaceStates[snapshot.workspaceId] ?? emptyWorkspaceState();
  ...
  abortSession: async (sessionId, workspaceId) => {
    try {
      await fetch(buildProxyUrl(`session/${sessionId}/abort`, workspaceId), {
        method: "POST",
      });
  ...
  rejectQuestion: async (requestId, workspaceId) => {
    try {
      await fetch(buildProxyUrl(`question/${requestId}/reject`, workspaceId), {
        method: "POST",
      });
      set((state) =>
        updateWorkspace(state, workspaceId, (ws) => ({
  ...
  handleEvent: (event, sourceWorkspaceId) => {
    ...
    switch (eventType) {
      ...
      case "question.asked": {
        ...
        const wsId =
          findWorkspaceForSession(get().workspaceStates, question.sessionID) ??
          sourceWorkspaceId;
      ...
      case "question.rejected": {
        ...
        const ownerWsId = Object.entries(get().workspaceStates).find(([, ws]) =>
          ws.questions.some((q) => q.id === requestID),
        )?.[0];
        if (!ownerWsId) break;
        set((state) =>
  ...
  getActiveSessionStatus: () => {
    ...
    return (
      workspaceStates[activeWorkspaceId]?.sessionStatuses[activeSessionId] ??
      null
    );
  },
  ...
  getActiveTodos: () => {
    ...
    return (
      workspaceStates[activeWorkspaceId]?.todos[activeSessionId] ?? EMPTY_TODOS
    );
  },
  ...
  getUnifiedSessionStatuses: () => {
    ...
    if (
      ...
      entries.every(
        ([id, ws]) => _unifiedStatusesCacheRefs.get(id) === ws.sessionStatuses,
      )
    ) {
  ...
  getUnifiedLastViewedAt: () => {
    ...
    if (
      ...
      entries.every(
        ([id, ws]) => _unifiedLastViewedCacheRefs.get(id) === ws.lastViewedAt,
      )
    ) {
  ...
  getUnifiedPinnedSessionIds: () => {
    ...
    if (
      ...
      entries.every(
        ([id, ws]) => _unifiedPinnedCacheRefs.get(id) === ws.pinnedSessionIds,
      )
    ) {
  ...
  getSessionAgent: (sessionId) => {
    const { activeWorkspaceId, workspaceStates } = get();
    const wsId =
      activeWorkspaceId ?? findWorkspaceForSession(workspaceStates, sessionId);
    if (!wsId) return null;
