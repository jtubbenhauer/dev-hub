...
describe("SessionPickerDialog", () => {
  ...
  it("filters sessions by search query", async () => {
    ...
    expect(
      sessionButtons.some((btn) => btn.textContent?.includes("Fix the bug")),
    ).toBe(true);
    expect(
      sessionButtons.some((btn) => btn.textContent?.includes("Add feature")),
    ).toBe(false);
  });
  ...
  it("sorts sessions by most recently updated", async () => {
    ...
    const buttons = screen
      .getAllByRole("button")
      .filter((btn) => btn.textContent?.includes("Session"));
