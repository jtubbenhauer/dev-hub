...
import { getWorkspaceBehaviour, shouldSSEConnect } from '@/lib/workspaces/behaviour'
...
describe('getWorkspaceBehaviour', () => {
  ...
  it('merges partial behaviour with defaults — partial fields override, rest are defaults', () => {
    ...
    expect(result.inactiveHealthIntervalMs).toBe(DEFAULT_PROVIDER_BEHAVIOUR.inactiveHealthIntervalMs)
    expect(result.activeHealthIntervalMs).toBe(DEFAULT_PROVIDER_BEHAVIOUR.activeHealthIntervalMs)
    expect(result.gitStatusIntervalMs).toBe(DEFAULT_PROVIDER_BEHAVIOUR.gitStatusIntervalMs)
    expect(result.branchPollWhenInactive).toBe(DEFAULT_PROVIDER_BEHAVIOUR.branchPollWhenInactive)
    expect(result.resumeTimeSeconds).toBe(DEFAULT_PROVIDER_BEHAVIOUR.resumeTimeSeconds)
  })
