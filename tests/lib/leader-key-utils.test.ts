...
describe("leader-key-utils", () => {
  describe("matchesActivationKey", () => {
    ...
    it("matches Ctrl+Shift+Space", () => {
      const config: ActivationKeyConfig = {
        key: " ",
        ctrlKey: true,
        shiftKey: true,
      };
  ...
  describe("formatActivationKey", () => {
    it("formats Ctrl+Space for non-Mac", () => {
      expect(formatActivationKey({ key: " ", ctrlKey: true }, false)).toBe(
        "Ctrl+Space",
      );
    });
    ...
    it("formats Alt+Space for non-Mac", () => {
      expect(formatActivationKey({ key: " ", altKey: true }, false)).toBe(
        "Alt+Space",
      );
    });

    it("formats Alt+Space for Mac as ⌥Space", () => {
      expect(formatActivationKey({ key: " ", altKey: true }, true)).toBe(
        "⌥Space",
      );
    });
    ...
    it("formats Ctrl+Shift+Space for non-Mac", () => {
      expect(
        formatActivationKey({ key: " ", ctrlKey: true, shiftKey: true }, false),
      ).toBe("Ctrl+Shift+Space");
    });

    it("formats Ctrl+Shift+Space for Mac as ⌃⇧Space", () => {
      expect(
        formatActivationKey({ key: " ", ctrlKey: true, shiftKey: true }, true),
      ).toBe("⌃⇧Space");
    });
    ...
    it("formats Meta+; for non-Mac as Super+;", () => {
      expect(formatActivationKey({ key: ";", metaKey: true }, false)).toBe(
        "Super+;",
      );
    });

    it("uppercases single-char keys", () => {
      expect(formatActivationKey({ key: "k", ctrlKey: true }, false)).toBe(
        "Ctrl+K",
      );
    });
  });
  ...
  describe("findMatchingPresetId", () => {
    ...
    it("finds alt+space preset", () => {
      expect(findMatchingPresetId({ key: " ", altKey: true })).toBe(
        "alt+space",
      );
    });
  ...
  describe("isValidActivationKeyConfig", () => {
    it("validates a correct config", () => {
      expect(isValidActivationKeyConfig({ key: " ", ctrlKey: true })).toBe(
        true,
      );
    });
  ...
  describe("ACTIVATION_KEY_PRESETS", () => {
    ...
    it("includes the default activation key as first preset", () => {
      expect(
        isActivationKeyEqual(
          ACTIVATION_KEY_PRESETS[0].config,
          DEFAULT_ACTIVATION_KEY,
        ),
      ).toBe(true);
    });
