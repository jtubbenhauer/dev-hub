...
describe("git-review-store", () => {
  ...
  it("toggleReviewed adds a file to the reviewed set", () => {
    ...
    const result = useGitReviewStore.getState().getReviewedFiles("ws1:working:")
  ...
  it("different keys have independent reviewed sets", () => {
    ...
    const working = useGitReviewStore.getState().getReviewedFiles("ws1:working:")
    const branch = useGitReviewStore.getState().getReviewedFiles("ws1:branch:main")
  ...
  it("clearReviewed clears only the specified key", () => {
    ...
    expect(useGitReviewStore.getState().getReviewedFiles("key2").has("b.ts")).toBe(true)
  })
